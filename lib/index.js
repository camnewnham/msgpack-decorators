...
function key(index, collectionType) {
    return function (target, propertyName) {
        ...
        if (classType === undefined) {
            throw new Error("Type undefined for key ".concat(index, " on ").concat(target.prototype, ". Are your declarations and imports in the right order?"));
        }
        if (typeof classType === "function" &&
            ...
            if (!isMessagePack(classType)) {
                throw new Error("Class type ".concat(classType, " is not a messagepack type"));
            }
        }
            ...
            if (collectionType) {
                if (collectionType !== Boolean &&
                    ...
                    if (!isMessagePack(collectionType)) {
                        throw new Error("Collection type ".concat(collectionType, " is not a messagepack type"));
                    }
                }
            }
            else {
                console.warn("Collection type unspecified on at key ".concat(index, " on ").concat(target.constructor.name, ". If this collection does not contain primitive values, it will fail to deserialize. To remove this warning, specific the type as @key(x,MyType)"));
            }
        }
...
function union(key, objectType) {
    return function (baseType) {
        if (objectType === undefined) {
            throw new Error("Type undefined for key ".concat(key, " on a union object. Are your declarations and imports in the right order?"));
        }
